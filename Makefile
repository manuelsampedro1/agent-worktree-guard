...
smoke:
	...
	printf 'agent edit\n' > src/change.py; \
	PYTHONPATH="$(CURDIR)/src" python3 -m agent_worktree_guard check "$$snapshot" --allow "src/**" >/tmp/agent-worktree-guard-check.md; \
	printf 'changed by mistake\n' >> notes/user-draft.md; \
	if PYTHONPATH="$(CURDIR)/src" python3 -m agent_worktree_guard check "$$snapshot" --allow "src/**" >/tmp/agent-worktree-guard-blocked.md; then exit 1; else exit 0; fi
